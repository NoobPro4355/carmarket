...
async function deleteListing(id, url, env) {
  const clientId = String(url.searchParams.get('clientId') || '');
  const row = await env.DB.prepare('SELECT client_id FROM listings WHERE id = ?').bind(id).first();
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (match && request.method === 'DELETE') {
      return deleteListing(match[1], url, env);
    }
